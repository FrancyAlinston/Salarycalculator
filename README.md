...
- **Default Home Screen Shift Heatmap & 12h Overtime Shift Calibration (`ShiftHeatmapCard.kt` & `CalculatorScreen.kt`):** Interactive shift heatmap embedded directly on the main home screen as the primary default view—no dialogs or extra clicks needed. Features sequential `< Month Year >` navigation with 12-month chip bar, quick presets including 4-On 4-Off (12h), calibrated 12h Overtime shift cycle (`0h -> 8h -> 12h -> 0h`) with amber badge highlighting, live payroll cutoff countdown, and real-time two-way synchronization with basic hours and overtime take-home pay.
